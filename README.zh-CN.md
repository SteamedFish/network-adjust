...
NUMA 感知版本可用：`linux_ethernet_optimization_numa.sh`。

**⚠️ 警告：此版本完全由 AI 生成，未经生产环境测试。**

此变体自动检测每个网卡的 NUMA 节点，并仅将 IRQ/RPS/XPS 绑定到该节点的本地 CPU，避免跨 NUMA 节点内存访问的性能损失。

**与原版的主要区别：**
- 从 `/sys/class/net/<dev>/device/numa_node` 自动检测网卡的 NUMA 节点
- 仅将 IRQ/RPS/XPS 绑定到该 NUMA 节点的本地 CPU
- 如果 NUMA 检测失败，自动回退到原始的轮询行为
- 详细的警告信息、测试建议和已知限制请查看脚本头部注释

**推荐使用场景：**
- 多路服务器（2+ CPU，具有独立的 NUMA 域）
- NIC NUMA 局部性对性能影响显著的系统
- 能够在生产部署前进行充分测试的用户
