...
# ========================
# ⚠️  NUMA-AWARE VERSION - EXPERIMENTAL  ⚠️
#
# This is an AI-generated NUMA-aware variant of linux_ethernet_optimization.sh
#
# *** COMPLETELY UNTESTED - USE AT YOUR OWN RISK ***
#
# Changes from original:
# - Detects NIC's NUMA node via /sys/class/net/<dev>/device/numa_node
# - Binds IRQ/RPS/XPS only to CPUs local to that NUMA node
# - Automatically falls back to original round-robin behavior if detection fails
#
# Recommended testing procedure:
# 1. Run with -n (dry-run) first to see what changes would be made
# 2. Test on a non-production system with known NUMA topology
# 3. Verify IRQ/RPS/XPS bindings with:
#    - cat /proc/irq/<irq>/smp_affinity
#    - cat /sys/class/net/<dev>/queues/rx-*/rps_cpus
#    - cat /sys/class/net/<dev>/queues/tx-*/xps_cpus
# 4. Monitor performance and CPU usage patterns
# 5. Compare with original script's behavior
#
# Known limitations:
# - Does not handle CPU hotplug
# - Does not handle memory-only NUMA nodes
# - Does not consider NUMA node distances (assumes local=best)
#
# For production use, consider professional NUMA tuning tools:
# - tuned (Red Hat)
# - irqbalance with --hintpolicy=subset
#
# Original script: linux_ethernet_optimization.sh
# ========================
...
# ========================
# NUMA Detection Functions (NUMA-aware variant only)
# ========================

# Get NIC's NUMA node number
# Returns: NUMA node number (0, 1, 2...), or -1 if detection fails
get_nic_numa_node() {
	local dev="$1"
	local numa_file="/sys/class/net/${dev}/device/numa_node"
	...
	local numa_node
	numa_node=$(cat "$numa_file" 2>/dev/null) || return 1
...
# Returns: Space-separated CPU list (e.g., "0 1 2 3 16 17"), or empty if failed
get_numa_node_cpus() {
	local node="$1"
	...
	local cpulist
	cpulist=$(cat "$cpulist_file" 2>/dev/null) || return 1
...
# Parse CPU list format (e.g., "0-15,32-47" or "0,2,4,6") into space-separated list
# Returns: Space-separated CPU numbers (e.g., "0 1 2 3 ... 47")
parse_cpu_list() {
	local cpulist="$1"
	local result=""
...
generate_cpus_mask() {
	local number=$1
	local total=$2
	local cpu_filter="$3" # Optional: space-separated CPU list (NUMA-aware)

	local NR_CPU
	NR_CPU=$(get_number_of_cpus)

	# If cpu_filter provided, use only those CPUs (NUMA-aware mode)
	if [ -n "$cpu_filter" ]; then
		local -a filtered_cpus=($cpu_filter)
		local nr_filtered=${#filtered_cpus[@]}

		if [ "$nr_filtered" -eq 0 ]; then
			# Empty filter, fallback to all CPUs
			cpu_filter=""
		fi
	fi
	...
	mask=0
	if [ -n "$cpu_filter" ]; then
		# NUMA-aware: round-robin within filtered CPUs
		local -a filtered_cpus=($cpu_filter)
		local nr_filtered=${#filtered_cpus[@]}
		while [ "${number}" -lt "${NR_CPU}" ]; do
			local cpu_idx=$((number % nr_filtered))
			local target_cpu=${filtered_cpus[$cpu_idx]}
			temp_mask="$(generate_cpus_mask_for_one_cpu "${target_cpu}" false)"
			mask="$(bignum_calc default "${mask} | ${temp_mask}")"
			number="$((number + total))"
		done
	else
		# Original behavior: round-robin across all CPUs
		while [ "${number}" -lt "${NR_CPU}" ]; do
...
_optimize_packet_steering() {
	...
	# NUMA-aware: Detect NIC's NUMA node and get local CPUs
	local numa_node local_cpus
	numa_node=$(get_nic_numa_node "${eth_name}")
	if [ $? -eq 0 ] && [ "$numa_node" != "-1" ]; then
		local_cpus=$(get_numa_node_cpus "$numa_node")
		if [ -z "$local_cpus" ]; then
			if [ "${mode}" = "apply" ]; then
				echo "WARNING: Cannot get CPUs for NUMA node ${numa_node}, ${label} using all CPUs"
			fi
			local_cpus=""
		fi
	else
		if [ "${mode}" = "apply" ]; then
			echo "WARNING: NUMA detection failed for ${eth_name}, ${label} using all CPUs"
		fi
		local_cpus=""
	fi
	...
	while read -r queue; do
		...
		if [ "$NR_QUEUE" -lt "$NR_CPU" ]; then
			# Generate mask with NUMA-aware CPU filter
			best_mask="$(generate_cpus_mask "${queue_number}" "${NR_QUEUE}" "${local_cpus}")"
		else
			best_mask="0"
		fi
...
_optimize_irq_affinity() {
	...
	if [ $? -eq 0 ] && [ "$numa_node" != "-1" ]; then
		...
		if [ -n "$local_cpus" ]; then
			if [ "${mode}" = "apply" ]; then
				echo "INFO: ${eth_name} on NUMA node ${numa_node}, using local CPUs: ${local_cpus}"
			fi
		else
			if [ "${mode}" = "apply" ]; then
				echo "WARNING: Cannot get CPUs for NUMA node ${numa_node}, using all CPUs"
			fi
			local_cpus=""
		fi
	else
		if [ "${mode}" = "apply" ]; then
			echo "WARNING: NUMA detection failed for ${eth_name}, using all CPUs"
		fi
		local_cpus=""
	fi

	# Convert local_cpus to array for round-robin
	local -a cpu_array
	if [ -n "$local_cpus" ]; then
		cpu_array=($local_cpus)
	else
	...
	local nr_available_cpus=${#cpu_array[@]}
	...
	for irq in $(get_irq_of_ethernet_card "${eth_name}"); do
		while read -r queue; do
			...
			# Round-robin within available CPUs (NUMA-local or all)
			local target_cpu=${cpu_array[$((queue % nr_available_cpus))]}
			best_mask="$(generate_cpus_mask_for_one_cpu "${target_cpu}" true)"
