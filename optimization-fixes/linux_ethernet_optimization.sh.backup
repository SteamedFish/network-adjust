...
# if the script is sourced by other scirpts, or executed directly
...
# Arbitrary precision calculator with multiple backend fallback.
# Bash can only handle integers up to 64-bit, so for >64 CPU masks
# we need external tools. Fallback order: bash -> bc -> python3 -> calc
bignum_calc() {
...
# generate a cpuset CPU mask based on the current number and total number
# man 7 cpuset, go or FORMAT section for detailed format description
generate_cpus_mask() {
	local number=$1
	local total=$2
...
get_irq_of_ethernet_card() {
	...
	# The methord used here seems to be driver irrelevant, but not 100% sure
	# Needs more testing for it

	# the legacy irq file in device/irq is usaually not used, and not exist in /proc/irq/<number>
	# we have to check if the irq acutally exists
...
get_filtered_ethernet_card_list() {
	...
	while read -r ETH_NAME; do
		...
		if [ "${#device_exclude_list[@]}" -gt 0 ]; then
			devname="$(get_name_of_ethrnet_card "${ETH_NAME}")"
		...
		if [ "${#vendor_exclude_list[@]}" -gt 0 ]; then
			vendorname="$(get_vendor_of_ethrnet_card "${ETH_NAME}")"
...
Examples:
    ...
    # Optimize all the ethernet cards in the system, only show the modifications without actually do modificatioons
    $0 -n
...
if ! check_script_requrements; then
	exit 1
fi
...
(
	...
	while read -r ETH_NAME; do
		...
		if ! (run_by_systemd || "${dry_run}"); then
